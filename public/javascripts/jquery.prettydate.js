...
$.prettyDate = {
	...
	parse: function(time) {
		var s = (time || '').replace(/-/g,'/').replace(/[TZ]/g,' ').replace(/\.\d\d\d/g, '') + 'UTC';
		return new Date(s);
	},
...
$.fn.prettyDate = function(options) {
	...
	function format() {
		elements.each(function() {
			var date = $.prettyDate.parse(options.value.apply(this));
			var when = $.prettyDate.format(date);
			if ( when && $(this).text() != when )
				$(this).text( when ).attr('title',date);
		});
	}
	format();
	if (options.interval)
